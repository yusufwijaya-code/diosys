CREATE TABLE IF NOT EXISTS `ms_price_plan` (
    ...
    `billingPeriod`   VARCHAR(50)  NULL,          -- e.g. "/ project", "/ month"
    `badge`           VARCHAR(80)  NULL,           -- e.g. "Most Popular", "Best Value"
    `originalPrice`   DECIMAL(12,2) NULL,          -- NULL means no discount shown
    `discountPercent` TINYINT UNSIGNED NULL,        -- e.g. 20 (for 20% off badge)
